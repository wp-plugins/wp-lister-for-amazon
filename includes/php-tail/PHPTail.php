		...
		<html>
			<head>
				...
				<script>
					...
					scroll = true;
					lastTab = window.location.hash != "" ? window.location.hash.substr(1) :  "<?php echo reset(array_keys($this->log)); ?>";
					console.log(lastTab);
					...
					function updateLog() {
						var tab = $('#'+lastTab);
						wp_ajax_url = 'admin-ajax.php?action=wpla_tail_log&ajax=1';
						$.getJSON( wp_ajax_url + '&tab=' + lastTab + '&lastsize=' + tab.data('lastSize') + '&grep='+tab.data('grep') + '&invert='+tab.data('invert'), function(data) {
				...
				<div id="tabs">
					...
					<div id="<?php echo $title;?>" data-lastSize="0" data-grep="" data-invert="0">
						<div class="header float">
							...
							<button class="clearLog">Clear</button>
							<span>Tailing file: <?php echo $file; ?></span> (<?php echo round(filesize($file)/1024,2); ?> kb) | <span class="grepspan">Grep keyword: ""</span> | <span class="invertspan"><!-- Inverted: false --></span>
						</div>
