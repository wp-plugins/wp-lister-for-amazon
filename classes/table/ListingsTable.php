...
class WPLA_ListingsTable extends WP_List_Table {
    ...
    function column_listing_title($item){
        ...
        $actions = array(
            ...
            'create_product'  => sprintf('<a href="?page=%s&action=%s&listing=%s">%s</a>',$page,'create_product',$item['id'],__('Create Product','wpla')),
            ...
            'end_item'        => sprintf('<a href="?page=%s&action=%s&listing=%s">%s</a>',$page,'end_item',$item['id'],__('End Listing','wpla')),
            ...
            'resubmit'        => sprintf('<a href="?page=%s&action=%s&listing=%s">%s</a>',$page,'resubmit',$item['id'],__('Submit again','wpla')),
            'delete'          => sprintf('<a href="?page=%s&action=%s&listing=%s">%s</a>',$page,'delete',$item['id'],__('Delete','wpla')),
    ...
    function column_lowest_price($item){
        ...
        $last_updated = $item['pricing_date'] ? human_time_diff( strtotime($item['pricing_date']) ) . ' ago' : '';
    ...
    function getPriceForItem($item) {        
        ...
        $value   = $product->regular_price;          // WC2.0 compat
    ...
    function getSalePriceForItem($item) {        
        ...
        $value   = $product->sale_price;          // WC2.0 compat
        $value   = $profile ? $profile->processProfilePrice( $value ) : $value;
    ...
    function column_status($item){
        ...
        switch( $item['status'] ){
            ...
            case 'online':
                ...
                if ( ! $item['quantity'] && ! $item['fba_quantity'] ) {
    ...
    function getProduct( $post_id ) {
        ...
        if ( $this->last_product_id != $post_id ) {
            $this->last_product_object = get_product( $post_id );
    ...
    function get_bulk_actions() {
        $actions = array(
            ...
            'get_compet_price'    => __('Fetch latest prices from Amazon','wpla'),
            'resubmit'            => __('Resubmit items','wpla'),
            'wpla_change_profile' => __('Change profile','wpla'),
            'trash_listing'       => __('Remove from Amazon','wpla'),
            'delete'              => __('Delete from database','wpla'),
            'get_lowest_offers'   => __('Get lowest offer listings','wpla') . ' (beta)',
            'fetch_pdescription'  => __('Fetch full description','wpla') . ' (beta)',
        );
        return $actions;
    }
