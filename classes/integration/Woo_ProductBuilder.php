...
class WPLA_ProductBuilder {
	...
	public function addProduct( $data ) {
		...
		// add amazon specific product meta
		// if ( $data['listing_type'] ) 	   update_post_meta( $post_id, '_amazon_auction_type',  $data['listing_type'] );
		// if ( $data['amazon_category_1_id'] ) update_post_meta( $post_id, '_amazon_category_1_id', $data['amazon_category_1_id'] );
		// if ( $data['amazon_category_2_id'] ) update_post_meta( $post_id, '_amazon_category_2_id', $data['amazon_category_2_id'] );

		// if ( $data['ShippingType'] ) update_post_meta( $post_id, '_amazon_shipping_service_type', $data['ShippingType'] );
		// if ( $data['ShippingPackage'] ) update_post_meta( $post_id, '_amazon_shipping_package', $data['ShippingPackage'] );
		// if ( $data['PackagingHandlingCosts'] ) update_post_meta( $post_id, '_amazon_PackagingHandlingCosts', $data['PackagingHandlingCosts'] );
		// if ( $data['InternationalPackagingHandlingCosts'] ) update_post_meta( $post_id, '_amazon_InternationalPackagingHandlingCosts', $data['InternationalPackagingHandlingCosts'] );
		// if ( $data['ShippingServiceOptions'] ) update_post_meta( $post_id, '_amazon_loc_shipping_options', $data['ShippingServiceOptions'] );
		// if ( $data['InternationalShippingServiceOption'] ) update_post_meta( $post_id, '_amazon_int_shipping_options', $data['InternationalShippingServiceOption'] );
	...
	public function updateProduct( $product_id, $data ) {
		...
		if ( ( $product->stock != $products_quantity ) && ( $product->product_type != 'variable' ) ) {
