...
class WPLA_AjaxHandler extends WPLA_Core {
	...
	public function ajax_wpla_use_lowest_price() {
		...
		if ( isset( $_REQUEST['id'] ) ) {
			...
			$product      = get_product( $listing['post_id'] );
			...
			if ( $listing ) {
				...
				if ( is_array( $pricing_info ) ) {
					...
					WPLA()->pages['listings']->display( 'apply_lowest_price', $tpldata );
	...
	public function ajax_wpla_update_price_column() {
		...
		if ( isset( $_REQUEST['listing_id'] ) ) {
			...
			// check column
			if ( ! in_array( $column, array('price','sale_price','min_price','max_price') ) ) return 'invalid column!';
