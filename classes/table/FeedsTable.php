...
class WPLA_FeedsTable extends WP_List_Table {
    ...
    function column_details($item){
        ...
        $actions = array(
            'view_amazon_feed_details' => sprintf('<a href="admin-ajax.php?action=%s&id=%s&sig=%s" target="_blank">%s</a>','wpla_feed_details',$item['id'],$signature,__('Details','wpla')),
            ...
            'view_amazon_feed_results' => sprintf('<a href="?page=%s&action=%s&amazon_feed=%s&width=600&height=470" class="thickbox">%s</a>',$_REQUEST['page'],'view_amazon_feed_results',$item['id'],__('View Results','wpla')),
            ...
            'submit_feed_to_amazon'    => sprintf('<a href="?page=%s&action=%s&amazon_feed=%s">%s</a>',$_REQUEST['page'],'submit_feed_to_amazon',$item['id'],__('Submit','wpla')),
            ...
            'process_amazon_feed_results'      => sprintf('<a href="?page=%s&action=%s&amazon_feed=%s">%s</a>',$_REQUEST['page'],'process_amazon_feed_results',$item['id'],__('Process Results','wpla')),
            'process_amazon_feed_again'        => sprintf('<a href="?page=%s&action=%s&amazon_feed=%s">%s</a>',$_REQUEST['page'],'process_amazon_feed_results',$item['id'],__('Process again','wpla')),
    ...
    function column_SubmittedDate($item){
        ...
        return sprintf('%1$s <br><span style="color:silver">%2$s</span>',
            ...
            /*$2%s*/ sprintf( __('%s ago','wpla'), human_time_diff( strtotime( $item['SubmittedDate'] ) ) )
        );
    }
    ...
    function get_columns(){
        $columns = array(
            ...
            'FeedSubmissionId'   => __('Feed ID','wpla'),
    ...
    function get_bulk_actions() {
        $actions = array(
            ...
            'delete'             => __('Delete selected feeds','wpla')
        );
        return $actions;
    }
